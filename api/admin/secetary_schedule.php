...
class SecretarySchedule {
    private function normalizeRole($role) {
        $role = strtolower(trim((string) $role));
        return preg_replace('/[\s_-]+/', ' ', $role);
    }

    private function requireSecretaryAccess() {
        if (session_status() === PHP_SESSION_NONE) {
            session_start();
        }

        if ($this->normalizeRole($_SESSION['user_role'] ?? '') !== 'secretary') {
...
$scheduleHandler = new SecretarySchedule();
