...
const summaryApiUrl = '../../api/student/payment.php?operation=getPrePlayPaymentDueSummary';
...
async function fetchPaymentDueSummary() {
    try {
        ...
        if (response.data?.status !== 'success') {
            throw new Error(response.data?.message || 'Unable to fetch student pre-play payment summary');
        }
        ...
        console.error('Error fetching student pre-play payment summary:', error);
        paymentDueSummary = {
            pending: [],
            due_today: [],
            due_tomorrow: [],
...
function buildDueRows(items, category) {
    ...
    return items.map(item => {
        ...
        const statusLabel = category === 'pending_review'
            ? (item.status || 'Pending')
            : (item.status || 'Unpaid');
        const dateLabel = category === 'pending_review'
...
function openDueRowAction(row) {
    ...
    if (row.dataset.category === 'pending_review') {
        if (typeof window.openPrePlayPaymentHistoryModal === 'function') {
            window.openPrePlayPaymentHistoryModal(enrollmentId, true);
        }
        return;
    }
