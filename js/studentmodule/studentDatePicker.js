export function initStudentDatePicker(options = {}) {
    ...
    if (!dateInput || !hiddenDate || !btn || !container) return;

    if (!window.__studentDatePickerInstances) {
        window.__studentDatePickerInstances = new Map();
    }

    const instanceKey = `${dateInputId}|${hiddenDateId}|${buttonId}|${containerId}`;
    if (window.__studentDatePickerInstances.has(instanceKey)) {
        if (container.style.display === 'block') {
            renderCalendar();
        }
        return;
    }
    window.__studentDatePickerInstances.set(instanceKey, true);
    ...
    function renderCalendar() {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if (isAvailable && window.teacherAvailableSlotsPerDate && window.teacherFullShiftsPerDate) {
                ...
                if (full && available && typeof window.parseTimeToMinutes === 'function') {
                    const fullMin = window.parseTimeToMinutes(full.end) - window.parseTimeToMinutes(full.start);
                    const availMin = available.reduce((sum, a) => sum + (window.parseTimeToMinutes(a.end) - window.parseTimeToMinutes(a.start)), 0);
...
window.setupStudentDatePicker = initStudentDatePicker;
