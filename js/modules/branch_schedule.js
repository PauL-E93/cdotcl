...
const BRANCH_SCHEDULE_ENDPOINT = '../../api/admin/branch_schedule.php';
const SEND_NOTIFICATION_ENDPOINT = '../../api/send_schedule_notification.php';
const ALL_FILTERS = 'all';

export default class BranchAdminCalendarModule {
    ...
    init() {
        this.container.innerHTML = `
            ...
            <div class="notification-wrapper">
                <div class="notif-header schedule-log-header">
                    <span>BRANCH SCHEDULE LOG</span>
                    <button type="button" id="branch-notify-all" class="schedule-notify-all-btn" title="Email everyone in this schedule log" aria-label="Email everyone in this schedule log">
        ...
        this.container.querySelector('#branch-notify-all').addEventListener('click', () => {
    ...
    bindPageFilters() {
        const teacherSelect = document.getElementById('branch-schedule-teacher-filter');
        const statusSelect = document.getElementById('branch-schedule-status-filter');
        ...
        teacherSelect?.addEventListener('change', () => {
            this.filters.teacher = teacherSelect.value || ALL_FILTERS;
            this.renderCalendar();
        });
    ...
    async loadBranchSchedules() {
        try {
            const response = await axios.get(`${BRANCH_SCHEDULE_ENDPOINT}?operation=getSchedules`);
            const data = response.data;
            const rawSchedules = data && Array.isArray(data.schedules)
                ? data.schedules
                : (Array.isArray(data) ? data : []);

            this.branchSchedules = rawSchedules.map(schedule => this.normalizeScheduleRow(schedule));
            this.teachers = Array.isArray(data?.teachers)
                ? this.normalizeTeacherList(data.teachers)
                : this.extractTeachers(this.branchSchedules);
            this.populateTeacherFilter();
            this.visibleSchedules = this.applyFilters(this.branchSchedules);
        } catch (error) {
    ...
    normalizeScheduleRow(row) {
        return {
            ...
            teacher_id: row.teacher_id === null || row.teacher_id === undefined ? '' : String(row.teacher_id),
    ...
    normalizeTeacherList(teachers = []) {
        const teacherMap = new Map();

        teachers.forEach(teacher => {
            const teacherId = String(teacher.employee_id ?? teacher.teacher_id ?? '').trim();
            const teacherName = String(teacher.teacher_name ?? teacher.name ?? teacher.teacher ?? '').trim();
            if (!teacherId || !teacherName || teacherMap.has(teacherId)) return;
    ...
    extractTeachers(schedules) {
        return this.normalizeTeacherList(
            schedules.map(schedule => ({
                employee_id: schedule.teacher_id,
    ...
    populateTeacherFilter() {
        const select = document.getElementById('branch-schedule-teacher-filter');
        if (!select) return;

        const currentValue = select.value || this.filters.teacher || ALL_FILTERS;
        const options = [
            '<option value="all">All Teachers</option>',
            ...this.teachers.map(teacher => {
                const teacherId = this.escapeHtml(teacher.employee_id);
                const teacherName = this.escapeHtml(teacher.teacher_name);
                return `<option value="${teacherId}">${teacherName}</option>`;
            })
        ];

        select.innerHTML = options.join('');
        const hasCurrent = Array.from(select.options).some(option => option.value === currentValue);
        select.value = hasCurrent ? currentValue : ALL_FILTERS;
        this.filters.teacher = select.value || ALL_FILTERS;
    }

    applyFilters(rows) {
        const teacher = this.filters.teacher || ALL_FILTERS;
        const status = this.filters.status || ALL_FILTERS;
        const search = this.filters.search || '';

        return rows.filter(schedule => {
            const matchesTeacher = teacher === ALL_FILTERS || schedule.teacher_id === teacher;
        ...
        days.forEach(dayElement => {
            ...
            if (daySchedules.length > 0) {
                ...
                dayElement.title = daySchedules
                    .map(schedule => `Subject: ${schedule.subject || 'Scheduled'} | Student: ${schedule.student || 'TBA'} | Teacher: ${schedule.teacher || 'TBA'}`)
                    .join('\n');
            }
        });
    }
    ...
    async renderCalendar() {
        await this.loadBranchSchedules();
    ...
    updateScheduleLog() {
        ...
        const notifyAllButton = this.container.querySelector('#branch-notify-all');
        if (notifyAllButton) {
    ...
    attachLogCardListeners() {
        const sessionButtons = this.container.querySelectorAll('.branch-schedule-session');
        ...
        const notifyButtons = this.container.querySelectorAll('.branch-schedule-notify');
    ...
    async updateScheduleStatus(preferenceId, enrollmentDetailsId, scheduleDate, newStatus) {
        ...
        try {
            const response = await axios.post(BRANCH_SCHEDULE_ENDPOINT, payload);
