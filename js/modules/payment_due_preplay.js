...
const summaryApiUrl = '../../api/admin/billing.php?operation=getPrePlayPaymentDueSummary';
...
async function fetchPaymentDueSummary() {
    try {
        ...
        if (response.data?.status !== 'success') {
            throw new Error(response.data?.message || 'Unable to fetch preplay payment due summary');
        }
        ...
        console.error('Error fetching preplay payment due summary:', error);
...
async function sendPaymentDueNotification(category) {
    ...
    try {
        ...
        if (response.data?.status !== 'success') {
            throw new Error(response.data?.message || 'Failed to send preplay payment notifications');
        }

        Swal.fire({
            icon: 'success',
            title: 'Email Sent',
            text: response.data.message || 'Preplay payment notification email sent successfully.'
        });
    } catch (error) {
        console.error('Error sending preplay payment notification email:', error);
        ...
        Swal.fire({
            icon: 'error',
            title: 'Send Failed',
            text: error.response?.data?.message || error.message || 'Unable to send the preplay payment notification email.'
        });
    }
}
